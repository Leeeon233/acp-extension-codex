...
- Steer uses app-server `turn/steer` on the tracked active turn. Correlate `clientUserMessageId` and acknowledge only the matching `item/completed(userMessage)`; never emulate steer with a second `turn/start`.
  A Core steer response of `outcome: "failed"` is a conclusive negative
  acknowledgement: return it only when the adapter can prove the prompt was
  not applied (currently an absent active turn or Codex's explicit "no active
  turn to steer" answer). Drain queued session notifications before classifying
  a `turn/steer` error, and let a matching completed user message win over that
  error. Transport/internal failures without application evidence must throw;
  callers must not automatically replay an ambiguous prompt.
